            ...
            <div className="animate-fade-in-up animate-delay-1" style={{ opacity: 0, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", flexWrap: "wrap", gap: "10px" }}>
                <div style={{ display: "flex", borderRadius: "12px", overflow: "hidden", border: "1px solid var(--color-border)" }}>
                    {(["news", "schemes", "diseases"] as const).map((tabId) => (
                        <button key={tabId} onClick={() => setTab(tabId)} style={{
                            ...
                            {tabId === "news" ? "📰 " + t.news.newsTab : tabId === "schemes" ? "🏛️ " + t.news.schemesTab : "🦠 " + t.news.diseaseTab}
                        </button>
                    ))}
                </div>
                <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                    ...
                    <button onClick={handleFetchAndSummarize} className="btn-primary" disabled={fetching} style={{ padding: "8px 16px", fontSize: "0.8rem" }}>
                        {fetching ? "⏳ " + t.mandi.fetching : "🔄 " + t.mandi.fetchLatest}
                    </button>
                </div>
            </div>
            ...
            {tab === "schemes" && schemes.some((s) => s.is_urgent) && (
                <div className="glass-card" style={{
                    ...
                    <p style={{ fontWeight: 700, color: "#ef4444", fontSize: "0.9rem", marginBottom: "6px" }}>
                        🔔 {t.news.urgent}: {t.news.deadlineWithin.replace("{count}", schemes.filter((s) => s.is_urgent).length.toString())}
                    </p>
                    ...
                    {articles.length === 0 && (
                        <div className="glass-card" style={{ padding: "48px", textAlign: "center" }}>
                            ...
                            <button onClick={handleFetchAndSummarize} className="btn-primary" disabled={fetching}>
                                {fetching ? "⏳ " + t.mandi.fetching : "🔄 " + t.mandi.fetchLatest + " " + t.news.newsTab}
                            </button>
                        </div>
                    )}
                                    ...
                                    <div style={{ marginTop: "14px", borderTop: "1px solid var(--color-border)", paddingTop: "14px" }}>
                                        ...
                                        {article.key_points && article.key_points.length > 0 && (
                                            <div style={{ marginBottom: "12px" }}>
                                                <p style={{ fontWeight: 600, fontSize: "0.85rem", color: "var(--color-text-main)", marginBottom: "6px" }}>📌 {t.news.keyPoints}:</p>
                                        ...
                                        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", alignItems: "center" }}>
                                            {language !== "en" && !trans && (
                                                ...
                                                >
                                                    {translatingId === article.id ? "⏳ " + t.news.translating : `🌐 ${t.news.translateTo.replace("{lang}", LANGUAGES[language])}`}
                                                </button>
                                            )}
                                            {article.url && (
                                                <a
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "10px" }}>
                                {scheme.eligibility && (
                                    <div style={{ background: "var(--color-bg-secondary)", borderRadius: "10px", padding: "12px", border: "1px solid var(--color-border)" }}>
                                        <p style={{ fontSize: "0.72rem", color: "var(--color-text-dim)", marginBottom: "4px", fontWeight: 600 }}>👤 {t.news.eligibility}</p>
                                        <p style={{ fontSize: "0.82rem", color: "var(--color-text-muted)" }}>{scheme.eligibility}</p>
                                    </div>
                                )}
                                {scheme.deadline && (
                                    <div style={{ background: "var(--color-bg-secondary)", borderRadius: "10px", padding: "12px", border: "1px solid var(--color-border)" }}>
                                        <p style={{ fontSize: "0.72rem", color: "var(--color-text-dim)", marginBottom: "4px", fontWeight: 600 }}>📅 {t.news.deadline}</p>
                            ...
                            >
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        {disease.affectedCrops?.length > 0 && (
                                            <div style={{ marginBottom: "10px" }}>
                                                <p style={{ fontSize: "0.75rem", color: "var(--color-text-dim)", fontWeight: 600, marginBottom: "4px" }}>🌾 {t.news.affectedCrops}:</p>
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "10px", marginTop: "14px" }}>
                                    {disease.symptoms?.length > 0 && (
                                        <div style={{ background: "var(--color-bg-secondary)", borderRadius: "10px", padding: "12px", border: "1px solid var(--color-border)" }}>
                                            <p style={{ fontSize: "0.75rem", color: "var(--color-text-dim)", fontWeight: 600, marginBottom: "6px" }}>🔍 {t.news.symptoms}</p>
                                    ...
                                    {disease.cureSteps?.length > 0 && (
                                        <div style={{ background: "var(--color-bg-secondary)", borderRadius: "10px", padding: "12px", border: "1px solid var(--color-border)" }}>
                                            <p style={{ fontSize: "0.75rem", color: "#22c55e", fontWeight: 600, marginBottom: "6px" }}>💊 {t.news.cureSteps}</p>
                                    ...
                                    {disease.pesticides?.length > 0 && (
                                        <div style={{ background: "var(--color-bg-secondary)", borderRadius: "10px", padding: "12px", border: "1px solid var(--color-border)" }}>
                                            <p style={{ fontSize: "0.75rem", color: "#f59e0b", fontWeight: 600, marginBottom: "6px" }}>🧪 {t.news.pesticides}</p>
                                            <ul style={{ paddingLeft: "16px", margin: 0 }}>
                                    ...
                                    {disease.organicRemedies?.length > 0 && (
                                        <div style={{ background: "var(--color-bg-secondary)", borderRadius: "10px", padding: "12px", border: "1px solid var(--color-border)" }}>
                                            <p style={{ fontSize: "0.75rem", color: "#22c55e", fontWeight: 600, marginBottom: "6px" }}>🌿 {t.news.organicRemedies}</p>
                                            <ul style={{ paddingLeft: "16px", margin: 0 }}>
                                    ...
                                    {disease.preventionTips?.length > 0 && (
                                        <div style={{ background: "var(--color-bg-secondary)", borderRadius: "10px", padding: "12px", border: "1px solid var(--color-border)" }}>
                                            <p style={{ fontSize: "0.75rem", color: "#3b82f6", fontWeight: 600, marginBottom: "6px" }}>🛡️ {t.news.prevention}</p>
                                            <ul style={{ paddingLeft: "16px", margin: 0 }}>
